...
def connection_requests():
    ...
    while True:
        ...
        if len(clients_connected) == 4:
            client_socket.send('non_autorisé'.encode())

            client_socket.close()
            continue
        else:
            client_socket.send('autorisé'.encode())

        try:
            client_name = client_socket.recv(1024).decode('utf-8')
        except:
            print(f"{address} deconnecté")
            client_socket.close()
            continue

        print(f"{address} a pour nom {client_name}")
        ...
        if client_socket.recv(1024).decode() == 'image_received':
            ...
            for client in clients_connected:
                if client != client_socket:
                    ...
                    data = pickle.dumps(
                        {'message': f"{clients_connected[client_socket][0]} a rejoint le chat", 'extension': image_extension,
...
def receive_data(client_socket):
    ...
    def disconnection():
        req = client_socket.rcv(255).decode("utf-8")     
        if  req == "finduchat":
            client_socket.close()
            print(f"{clients_connected[client_socket][0]} disconnected unexpectedly.")
connection_requests()
